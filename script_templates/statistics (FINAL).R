...
ggplot() +
  ...
  geom_line(data = sim_width1, aes(x = Year, y = Mean, color = "Simulation with prior a, c, and ζ"), size=1.5, lty=1.5) +
  geom_line(data = sim_width2, aes(x = Year, y = Mean, color = "Simulation with posterior a, c, and ζ"), size = 1.5) +
  geom_errorbar(data = sim_width2, aes(x = Year, ymin = Mean-SD, ymax = Mean+SD, color = "Simulation with posterior a, c, and ζ"),size=0.8) +
  scale_color_manual(values = c("Observation" = "#DAA520","Simulation with prior a, c, and ζ" = "#87CEEB", "Simulation with posterior a, c, and ζ" = "#4169E1")) +
...
heatscatter(Merge1$rw_obs, Merge1$rw_sim2, xlab = "Observed Ring Widths (mm)", ylab = "Simulated Ring Widths (mm)", main = "Heatscatter", xlim = c(0,15), ylim = c(0,15))
...
#################################################################################
# 02.01. general Muptiple Linear Regression with log_Age
#################################################################################
...
lm_sim1 <- lm (data = Merge1, rw_sim1 ~ tc + soilm + vpd + meanalpha + ppfd + co2 + log_Age)
summary(lm_sim1)
lm_sim2 <- lm (data = Merge1, rw_sim2 ~ tc + soilm + vpd + meanalpha + ppfd + co2 + log_Age)
summary(lm_sim2)
